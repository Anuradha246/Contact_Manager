...
const Contact = require('../models/Contact'); // Assuming your Contact model is in the models directory
...
router.post('/add-contact', async (req, res) => {
    const { name, phone, email, address, dob, info } = req.body;

    try {
        const newContact = new Contact({ name, phone, email, address, dob, info });
...
router.put('/update-contact/:id', async (req, res) => {
    ...
    try {
        const updatedContact = await Contact.findByIdAndUpdate(
            req.params.id,
            { name, phone, email, address, dob, info },
...
// Route to check for duplicate contacts by phone or email
